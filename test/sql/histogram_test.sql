...
-- table 2
CREATE TABLE "hitest2"(name varchar(30), score double precision, qualify boolean);

-- insertions
INSERT INTO "hitest2" VALUES('Tom', 6.2, TRUE);
INSERT INTO "hitest2" VALUES('Mary', 4.1, FALSE);
INSERT INTO "hitest2" VALUES('Jaq', 3.7, FALSE);
INSERT INTO "hitest2" VALUES('Jane', 10.5, TRUE);
...
-- standard 2 bucket with array 
SELECT qualify, histogram(score, array[0,5]) FROM hitest2 GROUP BY qualify;
-- standard multi-bucket 
SELECT qualify, histogram(score, 0, 10, 5) FROM hitest2 GROUP BY qualify;
-- standard multi-bucket with array 
SELECT qualify, histogram(score, array[0,2,4,6,8]) FROM hitest2 GROUP BY qualify;
-- standard element beneath lb
SELECT qualify, histogram(score, 5, 10, 5) FROM hitest2 GROUP BY qualify;
-- standard element beneath lb
SELECT qualify, histogram(score, array[5,6,7,8,9]) FROM hitest2 GROUP BY qualify;
-- standard element above ub
SELECT qualify, histogram(score, 0, 5, 3) FROM hitest2 GROUP BY qualify;
-- standard element above ub
SELECT qualify, histogram(score, array[0,2,4]) FROM hitest2 GROUP BY qualify;